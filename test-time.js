// Sprint 18 — Temporal Baseline Engine (TDD scaffolding)
//
// Standalone validator for `computeTemporalModifiers`. Once every scenario
// below prints PASS, the function gets ported verbatim into
...
function computeTemporalModifiers(dateObj) {
  ...
  // Morning Commute (Mon-Fri, 7:00 AM - 8:59 AM)
  if (day >= 1 && day <= 5 && hour >= 7 && hour <= 8) {
    rideMod = 1.5;
  }
  ...
  // Evening Commute (Mon-Fri, 4:00 PM - 5:59 PM)
  if (day >= 1 && day <= 5 && hour >= 16 && hour <= 17) {
    rideMod = 1.5;
  }

  // Dinner Rush (Every day, 5:00 PM - 7:59 PM)
  if (hour >= 17 && hour <= 19) {
    foodMod = 1.5;
  }

  // Weekend Bar Rush (Fri & Sat, 10:00 PM - 1:59 AM next day)
  // Late-night side: Fri (5) or Sat (6), hour >= 22
  if ((day === 5 || day === 6) && hour >= 22) {
    rideMod = 1.5;
    foodMod = 0.5;
  }
  // Early-AM wrap side: Sat (6) or Sun (0), hour < 2
...
// 2026-05-17 is a Sunday → day 0. Mon=18, Tue=19, Wed=20, Thu=21, Fri=22, Sat=23.
...
const cases = [
  ...
  {
    name: "Morning Commute — Monday 7:30 AM",
    date: mk(2026, 5, 18, 7, 30),
    expect: { foodMod: 1.0, rideMod: 1.5 },
  },
  {
  ...
  {
    name: "Evening Commute — Friday 4:30 PM",
  ...
  },
  {
    ...
    expect: { foodMod: 0.5, rideMod: 1.5 },
  },
  {
...
let allPass = true;
console.log("=== Sprint 18 Temporal Baseline — Test Run ===\n");
